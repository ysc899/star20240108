...
public class BagSendDetailViewModel extends AndroidViewModel {
    ...
    private NemoAPI api;
    private Handler saveHandler, deleteHandler, branchHandler, centerHandler, transportationHandler, transportCompanyHandler;


    public BagSendDetailViewModel(@NonNull Application application) {
        super(application);
...
//                        AndroidUtil.toast(application, "등록된 병원 정보가 없습니다.");
    ...
    {
        Date sendDt = this.getCpbgSendDt().getValue();

        param.setCpbgDprtDt(DateUtil.getFormatString(sendDt, "yyyyMMdd"));
    ...
    public void apiBagDelete(NemoBagSendListRO obj)
    {
        NemoBagSendDeletePO param = new NemoBagSendDeletePO();

        param.setCpbgSendDt(obj.getCpbgSendDt());
        param.setCpbgSendDeptCd(obj.getCpbgSendDeptCd());
        param.setCpbgDegCd(obj.getCpbgDegCd());
        param.setUserId(userId);

        AndroidUtil.log("NemoBagSendDeletePO : "+ param);


        api.deleteBagSend(param, deleteHandler);
    }
